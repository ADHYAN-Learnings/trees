...
import org.adhyan.hackerrank.trees.Result.Node;
...
public class BalancedForestTest {
    ...
    static long balancedForest(int[] node_values, int[][] edges) {
        ...
        System.out.println(" mini= "+mini+" ; sum="+sum);
     /**   for(Node node : nodes) {
    ...
    private static void solve(int p, ArrayList<Node> nodes) {
        ...
        long[] x = {2*node.cost, 2*sum - 4*node.cost, sum-node.cost};
        long[] y = {3*node.cost - sum,(sum-node.cost)/2 - node.cost};
        
        System.out.println(node.cost+" : "+x[0]+" , "+x[1]+" , "+x[2]);
        System.out.println(node.cost+" : "+y[0]+" , "+y[1]);
